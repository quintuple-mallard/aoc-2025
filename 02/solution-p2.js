...
function makeRanges(input) {
  ...
  for (const range of input.split(",")) {
    ...
    ranges.push(make_range(Number(start), Number(end)));
  }
  return ranges.flat(Infinity);
}
function invalid(num) {
  return /^(\d+)\1+$/.test(String(num));
}
function solve(input) {
  return makeRanges(input).filter(invalid).reduce((acc, el) => acc + el);
}
